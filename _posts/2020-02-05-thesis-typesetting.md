...
6. 序号后的空格问题
	...
	论文指南要求目录、标题、题注（图序、表序）中的序号和文字之间留一个汉字符。一个汉字符对应一个全角空格，两个英文空格。如果之前使用的是一个英文空格，可以根据样式选中文本进行批量替换操作。
	
	指南还要求题注的标签和序号之间不能有空格。需要注意的是Word对汉语的支持不好，添加的题注标签和序号之间必须有空格。这时也只有先选中所有这类文本后再删去这些空格。
